...
{
    threadpool_impl(size_t n) : crossplat::threadpool(n), m_work(asio::make_work_guard(m_service))
    {
...
private:
    ...
    {
        m_threads.push_back(
            std::unique_ptr<asio::detail::thread>(new asio::detail::thread([&] { thread_start(this); })));
    }
    ...
    std::vector<std::unique_ptr<asio::detail::thread>> m_threads;
...
struct shared_threadpool
{
    ...
    ~shared_threadpool()
    {
        ...
        bool terminate_threads = asio::detail::thread::terminate_threads();
        asio::detail::thread::set_terminate_threads(true);
        get_shared().~threadpool_impl();
        asio::detail::thread::set_terminate_threads(terminate_threads);
    }
};
